...
Breaking Changes:
^^^^^^^^^^^^^^^^^
- All customs environments (e.g. the ``BitFlippingEnv`` or ``IdentityEnv``) were moved to ``sb3f.common.envs`` folder
...
- Added `VecMonitor <https://github.com/DLR-RM/stable-baselines3/blob/master/sb3f/common/vec_env/vec_monitor.py>`_ and
  `VecExtractDictObs <https://github.com/DLR-RM/stable-baselines3/blob/master/sb3f/common/vec_env/vec_extract_dict_obs.py>`_ wrappers
...
Breaking Changes:
^^^^^^^^^^^^^^^^^
- Removed ``sb3f.common.cmd_util`` (already deprecated), please use ``env_util`` instead
...
- ``make_atari_env``, ``make_vec_env`` and ``set_random_seed`` must be imported with (and not directly from ``sb3f.common``):

.. code-block:: python

  from sb3f.common.cmd_util import make_atari_env, make_vec_env
